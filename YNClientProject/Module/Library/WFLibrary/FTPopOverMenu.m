...
#define FTDefaultMenuIconSize               10
    ...
    if (!menuImage) {
        ...
        [self getImageWithResource:menuImage
                        completion:^(UIImage *image) {
                            ...
                            self.iconImageView.width=image.size.width;
                            self.iconImageView.height=image.size.height;
                            self.iconImageView.y=(configuration.menuRowHeight-self.iconImageView.height)/2;
                        }];
...
- (UITableView *)menuTableView {
    if (!_menuTableView) {
        _menuTableView = [[GCBaseTableView alloc]initWithFrame:CGRectZero style:UITableViewStylePlain];
